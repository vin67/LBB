...
gulp.task('sassDev', ['clean'], function(){
  return gulp.src(paths.src.stylesheets+'/ionic.app.scss')
...
gulp.task('sassBuild', ['clean'], function(){
  //note no autoprefix as it breaks ionic css on android devices
  return gulp.src(paths.src.stylesheets+'/ionic.app.scss')
    .pipe(sass())
    .on('error', sass.logError)
    .pipe(minifyCss({
...
gulp.task('default', ['clean', 'copy', 'sassBuild', 'jsBuild']);

gulp.task('dev', ['clean', 'copy', 'sassDev', 'jsDev', 'watch']);